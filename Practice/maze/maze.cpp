// Miro Manestar
// January 1, 2021
//A simple maze implementation with a built in solver using BFS
...
public:
    ...
    bool check_dir(double x, double y, int direction, DisjointSet& set) {
        ...
        if (set.Find(i0) != set.Find(i1)) {
            set.Union(i0, i1);
            adj_list[i0].push_back(i1); adj_list[i1].push_back(i0);
            return true;
        } else {
            return false;
        }
        ...
        while (!queue.empty()) {
            ...
            for (int i = 0; i < (int) adj_list[v].size(); i++) {
                ...
                if (visited[adjV] == false) {
                    ...
                    if (adjV == (int) adj_list.size() - 1) {
                        int endCell = (int) adj_list.size() - 1;

                        while (endCell != 0) {
                            solution.push_back(endCell);
                            endCell = prev[endCell];
                        }
    ...
    void toggleSolution() {
        ...
        for (int i = 1; i < (int) solution.size(); i++) {
            double x0 = get_c(solution[i]) * width + width/2;
            double y0 = get_r(solution[i]) * height + height/2;
            double x1 = get_c(solution[i - 1]) * width + width/2;
            double y1 = get_r(solution[i - 1]) * height + height/2;
    ...
    int get_c(int i) { return (i % columns) + 1; }
    int get_r(int i) { return (i/columns) + 1; }
};
